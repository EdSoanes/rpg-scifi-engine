...
import { PlayerCharacter, State } from '../../lib/rpg-api/types'
...
export const statesSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    .addCase(fetchGraphState.fulfilled, (state, action) => {
      const playerCharacter = action.payload?.entities?.find((item) => item.archetype === 'PlayerCharacter') as PlayerCharacter
    ...
    .addCase(toggleState.fulfilled, (state, action) => {
      const playerCharacter = action.payload?.graphState.entities?.find((item) => item.archetype === 'PlayerCharacter') as PlayerCharacter
      const dict = playerCharacter?.states
      state.states = dict 
        ? Object.entries(dict).map((pair) => pair[1] as State)
        : []
