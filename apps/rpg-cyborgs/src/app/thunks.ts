import { createAsyncThunk } from "@reduxjs/toolkit"
import { RpgGraphState } from "../lib/rpg-api/types"
import { ActivityCreate, ActivityCreateRequest, ActivityResponse, SetState, SetStateRequest, StringResponse } from "../lib/rpg-api/server-types"
import { selectGraphState } from "./graphState/graphSelectors"
import { useSelector } from "react-redux"

export type ThunkStatus = 'idle' | 'loading' | 'loaded'

export const fetchGraphState = createAsyncThunk('graphState/fetch', async (id: string): Promise<RpgGraphState | undefined> => {
  const response = await get(`Cyborgs/PlayerCharacter/${id}`)
  const res = (await response.json()) as StringResponse
  return res?.graphState
})

export const fetchActivity = createAsyncThunk('activity/create', async (activityCreate: ActivityCreate): Promise<ActivityResponse | undefined> => {
  const graph = useSelector(selectGraphState)
  const op: ActivityCreateRequest = {
    graphState: graph!,
    op: activityCreate,
  }
  const response = await post('Cyborgs/activity/create', op)
  const res = (await response.json()) as ActivityResponse
  return res
})

export const toggleState = createAsyncThunk('state/toggle', async (setState: SetState): Promise<StringResponse | undefined> => {
  const graph = useSelector(selectGraphState)
  const setStateRequest: SetStateRequest = {
    graphState: graph!,
    op: setState,
  }

  const response = await post('Cyborgs/state', setStateRequest)
  const res = (await response.json()) as StringResponse
  return res
})

export const get = async (path: string) => {
  const response = await fetch(`https://localhost:44349/api/v1/rpg/${path}`, {
    method: 'GET',
    headers: {
      Accept: 'application/json; charset=utf-8',
      'Content-Type': 'application/json; charset=utf-8',
    },
  })

  return response
}

export const post = async (path: string, body?: unknown) => {
  const response = await fetch(`https://localhost:44349/api/v1/rpg/${path}`, {
    method: 'POST',
    headers: {
      Accept: 'application/json; charset=utf-8',
      'Content-Type': 'application/json; charset=utf-8',
    },
    body: body ? JSON.stringify(body) : null,
  })

  return response
}