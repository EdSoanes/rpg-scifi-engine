...
function StatPanel(props: StatPanelProps) {
  ...
  const [describe, setDescribe] = useState<PropDesc | undefined>()

  const eq = (props?.propValue?.value ?? 0) === (props?.propValue?.baseValue ?? 0)
  const inc = !eq && (props?.propValue?.value ?? 0) > (props?.propValue?.baseValue ?? 0)
  const dec = !eq && (props?.propValue?.value ?? 0) < (props?.propValue?.baseValue ?? 0)

  const onDescribe = async () => {
    if (props?.propValue) {
      const response = await getPropDesc(props.propValue.id, 'Value', graphState!)
