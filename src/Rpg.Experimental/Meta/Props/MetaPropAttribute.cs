...
namespace Rpg.Experimental.Meta.Props
{
    ...
    public abstract class MetaPropAttribute : Attribute
    {
        ...
        public MetaPropAttr GetValues()
        {
            ...
            foreach (var propInfo in GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(x => x.Name != "TypeId" && (x.PropertyType.IsPrimitive || x.PropertyType.IsEnum || RpgTypeUtilities.TypeNotExcluded(x.PropertyType))))
