...
namespace Rpg.SciFi.Engine
{
    ...
    public class DiceExpression
    {
        ...
        public List<KeyValuePair<int, IDiceExpressionNode>> nodes { get; } = new List<KeyValuePair<int, IDiceExpressionNode>>();
        ...
        private List<KeyValuePair<int, IDiceExpressionNode>> ParseToNodes(string expression)
        {
            ...
            for (int tokenIndex = 0; tokenIndex < tokens.Length; tokenIndex += 2)
            {
                ...
                {
                    var subExpr = SciFiEngine.Evaluate(nextToken.Replace("[", "").Replace("]", ""));

                    var subNodes = ParseToNodes(subExpr);
